...
class DefaultShell extends Shell {
  ...
  startProcess(shellPath, command, onStdOut, onStdErr, onExit) {
...
class TerminalShell extends Shell {
  ...
  startProcess(shellPath, command, onStdOut, onStdErr, onExit) {
    ...
    if (this.process) {
      ...
      this.disposable.add(this.process.onData(onStdOut))
...
export default class ProcessController {
  ...
  runProcessAfterProjectInput() {
    ...
    let shellPath = atom.config.get("process-palette.shell")
    ...
    try {
      this.process = this.shell.startProcess(shellPath, this.fields.fullCommand, onStdOut, onStdErr, onExit)
    } catch (e) {
      console.log(e)
    }
