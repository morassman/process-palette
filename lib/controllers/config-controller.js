...
module.exports =
(ConfigController = class ConfigController {
  ...
  constructor(projectController, config) {
    ...
    this.disposables.add(atom.commands.add(cssSelector, this.config.getCommandName(), this.runProcess));
    ...
    if (this.config.keystroke) {
      ...
      atom.keymaps.add(this.config.getCommandName(), bindings);
    }
  ...
  addMenus() {
    ...
    const root = {};
    let leaf = root;
    for (menu of Array.from(this.config.menus)) {
      const child = {};
      leaf['label'] = menu;
      leaf['submenu'] = [child];
      leaf = child;
    ...
    return this.disposables.add(atom.menu.add([root]));
  }
  ...
  recreateTreeViewMenuItem() {
    ...
    this.ctxMenuDisposables = new CompositeDisposable();
    this.ctxMenuDisposables.add(atom.commands.add('atom-workspace', commandName, () => this.runProcessFromTreeView()));
    ...
    return this.ctxMenuDisposables.add(atom.contextMenu.add({'.tree-view': [root]}));
  }
  ...
  recreateTreeViewMenuItemIfNeeded() {
    const hasCtxMenu = (this.ctxMenuDisposables != null);
    ...
    if (hasCtxMenu && !usesFileVars) {
      return this.disposeTreeViewMenuItem();
    } else if (!hasCtxMenu && usesFileVars) {
      return this.projectController.main.recreateTreeViewContextMenu();
    }
  }
  ...
  saveFile() {
    return this.projectController.saveFile();
  }
  ...
  setCommand(command) {
    ...
    return this.recreateTreeViewMenuItemIfNeeded();
  }
    // @saveFile();
  ...
  disposeTreeViewMenuItem() {
    if (this.ctxMenuDisposables != null) {
      this.ctxMenuDisposables.dispose();
    }
    return this.ctxMenuDisposables = null;
  }

  clearControllers() {
    return Array.from(this.processControllers).map((processController) =>
  ...
  runProcess() {
    ...
    return this.runProcessWithFile(filePath);
  }
  ...
  runProcessFromTreeView() {
    ...
    if (selected.length > 0) {
      return this.runProcessWithFile(selected[0].dataset.path);
    }
  }
  ...
  runProcessWithFile(filePath) {
    ...
    return processController.runProcessWithFile(filePath);
  }

  killRunningProcesses() {
    const clone = this.processControllers.slice(0);
    return Array.from(clone).map((pc) =>
      pc.killProcess());
  }
  ...
  removeProcessController(processController) {
    ...
    if (index !== -1) {
      ...
      return this.notifyProcessControllerRemoved(processController);
    }
  }
  ...
  removeOldest() {
    ...
    if (oldest !== null) {
      return this.removeProcessController(oldest);
    }
  }
  ...
  notifyProcessStarted(processController) {
    ...
    return _.invoke(_.clone(this.listeners), "processStarted", processController);
  }
  ...
  notifyProcessStopped(processController) {
    ...
    if ((this.config.outputTarget !== "panel") && (this.config.outputTarget !== "file")) {
      return this.removeProcessController(processController);
    }
  }

  notifyProcessControllerRemoved(processController) {
    return _.invoke(_.clone(this.listeners), "processControllerRemoved", processController);
  }
});
