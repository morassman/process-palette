...
class OutputView extends View {
  ...
  constructor(parent) {
    ...
    this.fontSize = atom.config.get('process-palette.outputPanel.fontSize')
...
class PanelOutputView extends OutputView {
  ...
  constructor(parent) {
    ...
    this.initialize()
    // atom.commands.add(this.refs.outputPanel, "core:copy", () => console.log('corecopy'))
  }
  ...
  render() {
    ...
    // native-key-bindings
    return <div ref="outputPanel" className="process-palette-process-output-view-panel" attributes={outputPanelAttributes}
...
class TerminalOutputView extends OutputView {
  ...
  render() {
    return <div ref="outputPanel" className="process-palette-process-output-view-terminal" />
  }

  processStarted() {
    this.xterm.onData(data => {
      if (this.processController.process) {
        this.processController.process.write(data)
      }
    })
  }
...
export default class ProcessOutputView extends View {
  ...
  initialize() {
    ...
    if (this.outputView instanceof TerminalOutputView) {
      this.refs.scrollLockButton.remove()
    }

    // The panel does not support pasting.
    if (this.outputView instanceof PanelOutputView) {
    ...
    this.outputPanel.classList.add('native-key-bindings')
