...
export default class ProcessOutputView extends View {
  ...
  render() {
    ...
    return <div className="process-palette-process-output-view">
      ...
      <div ref="outputPanel" className="process-palette-process-output-view-panel native-key-bindings" attributes={outputPanelAttributes} tabindex={1} on={{ mousedown: this.onOutputPanelMouseDown, scroll: this.onOutputPanelScroll, mousewheel: e => this.onOutputPanelMouseWheel(e) }} />
    </div>
  }
  ...
  appendLine(line) {
    for (let pattern of this.patterns) {
      ...
      if (match != null) {
        ...
        this.refs.outputPanel.insertAdjacentHTML("beforeend", this.sanitizeOutput(match.pre));
        this.refs.outputPanel.append(pathView.element)
        this.refs.outputPanel.insertAdjacentHTML("beforeend", this.sanitizeOutput(match.post));
        return;
      }
    }

    this.refs.outputPanel.insertAdjacentHTML("beforeend", this.sanitizeOutput(line));
  }
