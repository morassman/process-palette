...
export default class ProcessView extends View {
  ...
  render() {
    ...
    if (this.configController.config.outputToPanel()) {
      headerArgs.className = 'command-name inline-block text-highlight hand-cursor';
      ...
      outputValueArgs.className = 'hand-cursor';
      outputValueArgs.on = { click: this.showProcessOutput }
    } else {
      headerArgs.className = 'command-name inline-block text-highlight';
