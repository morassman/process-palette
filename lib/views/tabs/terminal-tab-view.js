...
class TerminalTabContentView extends View {
    ...
    return <div className="process-palette-terminal">
      <div ref="toolbar" className="process-palette-terminal-toolbar">
        <div className="process-palette-terminal-toolbar-title">
          <input ref="titleInput" class='input-text native-key-bindings' type='text' on={{ input: (e) => this.onTitleChange(e) }} />
        </div>
  ...
  initialize() {
    super.initialize()
    this.refs.titleInput.value = 'Terminal'
    this.addTooltips()
    this.startProcess()
  }
  ...
  onTitleChange(e) {
    this.tabView.setLabel(this.refs.titleInput.value)
  }
...
export default class TerminalTabView extends TabView {
  ...
  constructor(mainView) {
    super(true, 'terminal', 'Terminal', new TerminalTabContentView(mainView, () => this.onExit()), true)
