...
export default class CommandEditView extends View {
  ...
  textEditor(ref, mini) {
    this.tabindex++
    const textEditor = <TextEditor ref={ref} mini={mini} softTabs={true} softWrapped={true} lineNumberGutterVisible={false} />
  ...
  showConfig() {
    ...
    if (this.command.inputDialogs != null) {
      return Array.from(this.command.inputDialogs).map((inputDialog) =>
        this.refs.inputDialogsView.addRow([inputDialog.variableName, inputDialog.message, inputDialog.initialInput]));
    }
  }
