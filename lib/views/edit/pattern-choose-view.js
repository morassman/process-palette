...
class PatternItemView extends View {
  ...
  render() {
    return <div>
      <button className="btn btn-sm icon-arrow-up pattern-button" on={{ click: this.moveUp, mousedown: e => e.preventDefault() }} />
      <input ref="checkbox" type="checkbox" />
      <span className="pattern-name">{this.name}</span>
      <span className="text-subtle pattern-expression">{this.value.expression}</span>
    </div>
  }
...
export default class PatternChooseView extends View {
  ...
  render() {
    return <div>
      <div className="pattern-choose-view">
        <ul ref="list" className="list-group"></ul>
      </div>
    </div>
  }
