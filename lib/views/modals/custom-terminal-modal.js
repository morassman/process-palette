...
const { TextEditor, CompositeDisposable, Directory } = require('atom')
...
class CustomTerminalModal extends View {
  ...
  initialize() {
    ...
    this.refs.cwdCustomEditor.setText(cwdCustom)
    this.refs.tabTitleEditor.setText('Terminal')
    ...
    return <table className="process-palette-custom-terminal-cwd">
      <tbody>
        <tr on={{ click: () => this.selectCwd('cwdCustom') }}>
          <td>
            <label class='input-label'><input ref="cwdCustom" class='input-radio' type='radio' />Custom</label>
          </td>
          <td>
            <TextEditor ref="cwdCustomEditor" mini={true} />
          </td>
        </tr>
        <tr on={{ click: () => this.selectCwd('cwdCurrent') }}>
          <td>
            <label class='input-label'><input ref="cwdCurrent" class='input-radio' type='radio' />Current</label>
          </td>
          <td>{this.cwdCurrent}</td>
        </tr>
        <tr on={{ click: () => this.selectCwd('cwdHome') }}>
          <td>
            <label class='input-label'><input ref="cwdHome" class='input-radio' type='radio' />Home</label>
          </td>
          <td>{this.cwdHome}</td>
        </tr>
        {this.projects.map((project, i) => <tr on={{ click: () => this.selectCwd(`cwdProject${i}`) }}>
          <td>
            <label class='input-label'><input ref={`cwdProject${i}`} class='input-radio' type='radio' />{`Project: ${project.name}`}</label>
          </td>
          <td>{project.path}</td>
        </tr>)}
      </tbody>
    ...
    return <div className="process-palette-modal">
      ...
      <div className="process-palette-modal-body process-palette-custom-terminal">
        <div className="process-palette-custom-terminal-tab">
          <h2>Tab Title</h2>
          <TextEditor ref="tabTitleEditor" mini={true} />
        </div>
  ...
  getSelectedCwd() {
    ...
    if (name === 'cwdCustom') {
      return this.refs.cwdCustomEditor.getText().trim()
  ...
  confirm() {
    const title = this.refs.tabTitleEditor.getText().trim()
