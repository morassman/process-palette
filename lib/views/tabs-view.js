...
export class TabView extends View {
  ...
  render() {
    ...
    // if (this.closable) {
    //   children.push(<span>X</span>)
    // }

    if (this.icon) {
      children.push(<span className={`icon icon-${this.icon}`}></span>)
    }

    if (this.label) {
      children.push(<span>{this.label}</span>)
    }
...
export class ConfigTabView extends TabView {
  ...
  constructor(configController) {
    super(false, null, null, new ConfigTabContentView(), false)
    this.configController = configController
    this.currentProcessController = null
    this.label = _.humanizeEventName(configController.config.getCommandName())
    this.initialize()
  }
  ...
  render() {
    return <div className="process-palette-tabs-view" attributes={this.getAttributes()}>
      <div ref="header" className="process-palette-tabs-view-header">
        <div ref="tabs" className="process-palette-tabs-view-list"></div>
      </div>
      <div ref="content" className="process-palette-tabs-view-content"></div>
    </div>
  }
